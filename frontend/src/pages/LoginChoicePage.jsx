  ...
  return (
    <div className="login">
      ...
      <div className="login-main">
        <h2>Planti</h2>
        <button className="btn-login" onClick={() => navigate("/api/auth/login")}>
          Login
        </button>
        <button className="btn-register" onClick={() => navigate("/api/auth/signup")}>
          회원가입
        </button>
        <div className="notice">
          계속 진행하면 이용약관과 개인정보 처리방침에 동의하는 것으로 간주됩니다.
        </div>
        <div className="back" onClick={() => navigate("/api/auth/index")}>
          ← 처음 화면으로
        </div>
